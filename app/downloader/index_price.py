...
class IndexPriceDownloader:
    ...
    def download_range(
        ...
        after = None
        while True:
            page = self.client.get_index_candles(inst_id=inst_id, bar=bar, after=after, limit=100)
            if not page:
                break
        ...
        logger.info("IndexPrice 下载完成: %s | %s | %d 条", inst_id, bar, written)
        return written
    ...
    def _update_sync_state(self, inst_id: str, bar: str, latest_ts, earliest_ts) -> None:
        ...
        stmt = pg_insert(IndexPriceSyncState).values(row)
        stmt = stmt.on_conflict_do_update(
