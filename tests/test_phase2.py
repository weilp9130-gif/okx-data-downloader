...
class TestMarkPriceNormalize(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture_path = Path(__file__).resolve().parent / "fixtures" / "mark_price_candles.json"
...
class TestIndexPriceNormalize(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture_path = Path(__file__).resolve().parent / "fixtures" / "index_candles.json"
        with open(cls.fixture_path, "r", encoding="utf-8") as f:
            cls.raw_list = json.load(f)
...
class TestOpenInterestNormalize(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture_path = Path(__file__).resolve().parent / "fixtures" / "open_interest.json"
        with open(cls.fixture_path, "r", encoding="utf-8") as f:
            cls.raw_list = json.load(f)
